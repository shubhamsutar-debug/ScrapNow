...
export default function CollectorPickupRequests() {
  ...
  const handleAccept = (req: PickupRequest) => {
    ...
    setToastMessage(`✓ Pickup Request ${req.id} Accepted! Moved to My Pickups.`);
    ...
    <div className="min-h-screen bg-brand-bg flex flex-col justify-between">
      ...
      {selectedDetailsRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-brand-card border border-brand-border rounded-3xl p-6 sm:p-8 max-w-md w-full z-10 space-y-4 shadow-2xl relative">
            ...
            <h3 className="text-xl font-bold text-brand-text">Pickup Details #{selectedDetailsRequest.id}</h3>
            ...
            <div className="space-y-2 text-xs sm:text-sm border-y border-brand-border py-3">
              ...
              <div className="flex justify-between">
                <span className="text-brand-text-secondary">Time Slot:</span>
                <span className="font-semibold text-brand-text">{selectedDetailsRequest.timeSlot}</span>
              </div>
            </div>

            <div className="space-y-1">
              <span className="text-xs font-bold text-brand-text uppercase tracking-wider block">Items & Weight</span>
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className="w-full py-3 bg-brand-primary text-white font-bold text-xs rounded-xl hover:bg-brand-dark transition"
              >
                Accept Order
              </button>
            </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-brand-text">Pickup Requests</h1>
            <p className="text-brand-text-secondary text-xs sm:text-sm mt-0.5">
              Available scrap collection orders submitted by customers in {selectedCity}
            </p>
          </div>
          <button
